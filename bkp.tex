...
\keywordsCS{správa projektů, }
\keywordsEN{project management, ruby, redmine, chiliproject, rails, plugin, extension, module, test, monkey patching}
...
současné době se začíná prosazovat i na \glslink{FIT}{FIT}\footnote{\url{http://teambox.fit.cvut.cz/}}.
...
Ožehavá otázka je: Co všechno testovat? Obligátní odpověď \uv{všechno}
...
Protože veškerý kód v této práci je open source, rozšíření
jsou od začátku hostovaná ve veřejných repozitářích na serveru GitHub.
Není to pouze otázka osobní preference, existuje řada nezávislých služeb
které nabízí projektům na GitHubu užitečné funkce navíc.
V tomto případě jsem se rozhodl využít službu Travis-CI\footnote{\url{http://travis-ci.org/}},
která nabízí hostované řešení pro \gls{CI} open source projekty zcela zdarma.

Travis-CI funguje následovně: ze serveru GitHub přijde oznámení, že v cílovém
repozitáři přibyl nový \gls{commit} (přes tzv. post-commit hook). Služba
spustí \emph{build}: vytvoří virtuální stroj, stáhne do něj repozitář
s kódem, nainstaluje potřebné knihovny, spustí sadu testů. Testy
vrací návratový kód -- jak je typické pro Unix: \lstinline!0! znamená
úspěch, jiné číslo je neúspěch. Travis-CI kontroluje tento návratový kód a
...
odebírat členy. Ačkoliv to částečně řeší decentralizaci správy, stále je
...
v určitém projektu může chovat jako jiná. Ačkoliv to nezní moc užitečně,
