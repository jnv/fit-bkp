...
\documentclass[thesis=B,czech]{FITthesis}[2012/04/27]
...
\keywordsCS{ruby, redmine, chiliproject, tdd, rails}
\keywordsEN{ruby, redmine, chiliproject, tdd, rails}
...
name=Git,
description={decentralizovaný systém správy kódu}
...
name=commit,
description={sada změn přidaných do repozitáže kódu}
} \newglossaryentry{submodul}{
name=submodul,
description={v kontextu systému Git: odkaz na externí repozitář z lokálního repozitáře}
} \newglossaryentry{gem}{
name=gem,
description={v kontextu jazyka Ruby: správce balíčků s aplikacemi a knihovnami pro Ruby}
...
Největší problém je \uv{globálnost} -- role a skupiny může definovat
...
\uv{configuration over convention}. Síla Drupalu je však i největší
...
autor Ruby, Yukihiro \uv{Matz} Matsumoto, podobu jazyka ovlivnil i editor
...
paradigmatem \uv{convention over configuration} však předpokládá, že
...
Charakteristické pro Rails je, že databáze je považována za \uv{hloupé}
...
funkce aplikace prostřednictvímí dílčích \uv{miniaplikací} -- každý
...
tato idea příliš nefunguje -- výsledkem jsou často \uv{integrační testy v
přestrojení,} zbytečně dlouhé, hůře strukturované a přesto pro
...
\uv{testovacího náhradníka} (test double) objektu, což je užitečné v
...
Ožehavá otázka je: Co všechno testovat? Obligátní odpověď \uv{všechno} v
...
zapotřebí pouze jejich část. Výhodou je potenciálně \uv{čistší}
...
interakci\footnote{Rails od verze 3 používá \uv{unobtrusive JavaScript}
...
při přidávání uživatele do projektu, \uv{našeptávat} uživatele nalezené v
...
\begin{figure}
	\dirtree{%
		...
		.1 plugins\DTcomment{individuální rozšíření}.
