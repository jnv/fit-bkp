...
\hyphenation{Chili-Project Red-mine Active-Record}
...
name=Mercurial,
description={Distribuovaný systém správy kódu}
} \newglossaryentry{scmgls}{
name=SCM,
description={Souhrnné označení software pro správu kódu}
} \newglossaryentry{ldapgls}{
name=LDAP,
description={Adresářová služba}
...
} \newglossaryentry{saasgls}{
name=SaaS,
description={Software poskytovaný formou on-line služby}
...
} \newglossaryentry{aclgls}{
name=ACL,
description={Seznam uživatelů a jejich oprávnění pro přístup k~určitému zdroji}
...
\newglossaryentry{yamlgls}{
name=YAML,
description={Textový formát pro serializaci dat, zaměřený na uživatelskou přívětivost; viz \url{http://www.yaml.org/}}
}
\newacronym[description=\glslink{yamlgls}{YAML Ain't Markup Language}]{YAML}{YAML}{YAML Ain't Markup Language}
\newacronym[description=\glslink{aclgls}{Access Control List}]{ACL}{ACL}{Access Control List}
\newacronym[description=\glslink{saasgls}{Software as a Service}]{SaaS}{SaaS}{Software as a Service}
\newacronym[description=\glslink{ldapgls}{Lightweight Directory Access Protocol}]{LDAP}{LDAP}{Lightweight Directory Access Protocol}
\newacronym[description=\glslink{scmgls}{Source Code Management}]{SCM}{SCM}{Source Code Management}
...
v~současné době se začíná prosazovat i na \acrshort{FIT}.\footnote{\url{http://teambox.fit.cvut.cz/}}
...
ChiliProject je napsaný v~jazyce Ruby s~použitím frameworku
Ruby on Rails.
...
ActiveSupport (z~frameworku Rails).
...
serializovaná data (nejčastěji ve formátu \acrshort{YAML}, jenž je činní
...
tak i v~případě \acrfull{REE} \citep[End of Life]{REEEOL}, což je modifikovaná verze
\gls{MRI} řady 1.8, která by měla poskytovat vyšší výkon pro serverové aplikace
(a oproti oficiální verzi \gls{MRI} 1.8 se jí to skutečně daří).

Čísla verzí jsou u~\gls{MRI} velice zavádějící, aplikace napsané
pro \gls{MRI} verze 1.9.2 mohou mít problémy s~verzí 1.9.3.
ChiliProject používá mnoho knihoven, ktré byly cílené na \gls{MRI} řady
1.8 a v~současnosti už se nevyvíjí. Přitom se problémy s~kompatibilitou
poměrně obtížně analyzují, většinou jsou důsledkem drobné změny v~chování interpreta.

Cílová verze frameworku Rails je určená aplikací ChiliProject -- ta používá
poslední verzi Rails řady 2 (2.3.14), která už není oficiálně podporovaná.
...
Alternativou by bylo dále rozvíjet systém přidávání uživatelů,
který by umožnil vyhledávání a hromadný import uživatelů na základě
určitých kritérií -- například uživatelské skupiny v~\gls{LDAP}.
...
\item[LibreSource] \url{http://dev.libresource.org/}
\item[PHProjekt] \url{http://www.phprojekt.com/}
\item[TeamLab] \url{http://www.teamlab.com/}
...
\begin{figure}
	\dirtree{%
		...
		.2 src\DTcomment{zdrojové soubory práce pro \LaTeX{}}.
